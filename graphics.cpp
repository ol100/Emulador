...
/*unsigned int m_ScreenData[160][144][3];
	
//COLOUR framebuffer[160 * 144];

void dibujarScanline(){
	unsigned char control1= loadMEMB(0xFF40);
	if(TestBit(control1,0)){
		//std::cout<<"RENDER TILES"<<std::endl;
		renderTiles();
	}
	if(TestBit(control1,1)){
		std::cout<<"RENDER SPRITES"<<std::endl;
		RenderSprites();
	}
}
...
void renderTiles(){
	...
	unsigned char control1= loadMEMB(0xFF40);
	if(TestBit(control1,5)){
		if(windowY<=loadMEMB(0xFF44)){
			usingWindow=true;
		}
	}
	if(TestBit(control1,4)){
		tileData=0x8000;
	}
	else{
		tileData=0x8800;
	...
	for(int i=0;i<160;i++){
		...
		//std::cout<<"ROJO: "<<m_ScreenData[i][finaly][0]<<std::endl;
...
void RenderSprites()
{
   ...
   for (int sprite = 0 ; sprite < 40; sprite++)
   {
	 ...
	 std::cout<<"ATTRIBUTTES:"<<(int)attributes<<std::endl;
     ...
     if ((scanline >= yPos) && (scanline < (yPos+ysize)))
     {
       ...
       for (int tilePixel = 7; tilePixel >= 0; tilePixel--)
       {
         ...
         switch(col)
         {
           case WHITE: red =255;green=255;blue=255;std::cout<<"BLANCO: " <<std::endl;break ;
           case LIGHT_GRAY:red =0xCC;green=0xCC ;blue=0xCC;std::cout<<"GRIS: " <<std::endl;break ;
           case DARK_GRAY:red=0x77;green=0x77;blue=0x77;std::cout<<"GROS: " <<std::endl;break ;
         }
